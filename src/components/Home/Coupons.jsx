...
const Coupons = () => {
  ...
  const cardGradients = [
    "from-amber-500 to-pink-500",       // Warm sunset
    "from-emerald-400 to-cyan-500",     // Fresh teal
    "from-violet-500 to-purple-600",    // Deep purple
    "from-rose-500 to-red-500",         // Vibrant red
    "from-sky-400 to-blue-600",         // Classic blue
  ];
      ...
      <div className="container mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-base-content mb-4">
            Exclusive <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Deals</span>
          </h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-base-content/70 text-lg">No active coupons currently</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {coupons.map(({ _id, code, discount, description, expiryDate }, idx) => (
              <div
                key={_id}
                className={`relative rounded-2xl overflow-hidden bg-gradient-to-br ${cardGradients[idx % cardGradients.length]} p-0.5 shadow-xl hover:shadow-2xl transition-all duration-300`}
              >
